...
/*   Updated: 2022/05/28 16:39:24 by jrasser          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_exec_cmd(t_data *data)
{
	...
	if (data->inputs[0].child == -1)
		perror("Error");
	if (data->inputs[0].child == 0)
	{
		if (execve(data->inputs[0].cmd_fct, data->inputs[0].cmds, data->inputs[0].env) == -1)
		{
			if (data->inputs[0].cmds[0] != NULL)
